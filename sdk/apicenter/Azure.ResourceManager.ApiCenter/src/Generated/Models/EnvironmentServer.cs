// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System;
using System.Collections.Generic;
using Azure.ResourceManager.ApiCenter;

namespace Azure.ResourceManager.ApiCenter.Models
{
    /// <summary> Server information of the environment. </summary>
    public partial class EnvironmentServer
    {
        /// <summary>
        /// Keeps track of any properties unknown to the library.
        /// <para>
        /// To assign an object to the value of this property use <see cref="BinaryData.FromObjectAsJson{T}(T, System.Text.Json.JsonSerializerOptions?)"/>.
        /// </para>
        /// <para>
        /// To assign an already formatted json string to this property use <see cref="BinaryData.FromString(string)"/>.
        /// </para>
        /// <para>
        /// Examples:
        /// <list type="bullet">
        /// <item>
        /// <term>BinaryData.FromObjectAsJson("foo")</term>
        /// <description>Creates a payload of "foo".</description>
        /// </item>
        /// <item>
        /// <term>BinaryData.FromString("\"foo\"")</term>
        /// <description>Creates a payload of "foo".</description>
        /// </item>
        /// <item>
        /// <term>BinaryData.FromObjectAsJson(new { key = "value" })</term>
        /// <description>Creates a payload of { "key": "value" }.</description>
        /// </item>
        /// <item>
        /// <term>BinaryData.FromString("{\"key\": \"value\"}")</term>
        /// <description>Creates a payload of { "key": "value" }.</description>
        /// </item>
        /// </list>
        /// </para>
        /// </summary>
        private IDictionary<string, BinaryData> _serializedAdditionalRawData;

        /// <summary> Initializes a new instance of <see cref="EnvironmentServer"/>. </summary>
        public EnvironmentServer()
        {
            ManagementPortalUri = new ChangeTrackingList<Uri>();
        }

        /// <summary> Initializes a new instance of <see cref="EnvironmentServer"/>. </summary>
        /// <param name="serverType"> Type of the server that represents the environment. </param>
        /// <param name="managementPortalUri"> The location of the management portal. </param>
        /// <param name="serializedAdditionalRawData"> Keeps track of any properties unknown to the library. </param>
        internal EnvironmentServer(EnvironmentServerType? serverType, IList<Uri> managementPortalUri, IDictionary<string, BinaryData> serializedAdditionalRawData)
        {
            ServerType = serverType;
            ManagementPortalUri = managementPortalUri;
            _serializedAdditionalRawData = serializedAdditionalRawData;
        }

        /// <summary> Type of the server that represents the environment. </summary>
        public EnvironmentServerType? ServerType { get; set; }
        /// <summary> The location of the management portal. </summary>
        public IList<Uri> ManagementPortalUri { get; }
    }
}
